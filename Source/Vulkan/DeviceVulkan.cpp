	...
	namespace graphics
	{
		namespace vulkan
		{
			...
			public:
				...
				VkCommandBuffer GetCommandBuffer(size_t nIndex) const { return m_vecCommandBuffers[nIndex]; }
			...
			void Device::Impl::Run(std::function<void()> funcUpdate)
			{
			...
			{
				VkResult result = vkAcquireNextImageKHR(m_device, m_swapChain, std::numeric_limits<uint64_t>::max(), m_imageAvailableSemaphore, VK_NULL_HANDLE, &m_nFrameIndex);
				...
				VkCommandBuffer commandBuffer = m_vecCommandBuffers[m_nFrameIndex];
				...
				//
				//ImageBarrier(commandBuffer, m_vecSwapChainImages[m_nFrameIndex]->GetImage(),
				//	0,
				...
				//
				//vkCmdCopyImage(commandBuffer, pRenderTarget->GetImage(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_vecSwapChainImages[m_nFrameIndex]->GetImage(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyInfo);
				//
				//ImageBarrier(commandBuffer, m_vecSwapChainImages[m_nFrameIndex]->GetImage(), VK_ACCESS_MEMORY_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
				...
				submitInfo.pCommandBuffers = &m_vecCommandBuffers[m_nFrameIndex];
				...
				presentInfo.pImageIndices = &m_nFrameIndex;
			...
			void Device::Impl::InitializeWindow(uint32_t nWidth, uint32_t nHeight, bool isFullScreen, const String::StringID& strApplicationTitle, const String::StringID& strApplicationName)
			{
				...
				m_n2ScreenSize.x = nWidth;
				m_n2ScreenSize.y = nHeight;

				glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

				m_pWindow = glfwCreateWindow(static_cast<int>(nWidth), static_cast<int>(nHeight), strApplicationTitle.c_str(), nullptr, nullptr);
			...
			void Device::Impl::RecreateSwapChain()
			{
				int nWidth = 0;
				int nHeight = 0;
				glfwGetWindowSize(m_pWindow, &nWidth, &nHeight);
				if (nWidth == 0 || nHeight == 0)
					return;

				if (nWidth == static_cast<int>(m_n2ScreenSize.x) &&
					nHeight == static_cast<int>(m_n2ScreenSize.y))
					return;

				m_n2ScreenSize.x = static_cast<uint32_t>(nWidth);
				m_n2ScreenSize.y = static_cast<uint32_t>(nHeight);
			...
			void Device::Impl::CreateInstance()
			{
				...
				appInfo.pApplicationName = m_strApplicationName.c_str();
			...
			void Device::Impl::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t nWidth, uint32_t nHeight)
			{
				VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
			...
			void Device::Impl::CreateImage(uint32_t nWidth, uint32_t nHeight, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage* image_out, VkDeviceMemory* imageMemory_out)
			{
			...
			bool Device::Impl::CheckDeviceExtensionSupport(VkPhysicalDevice device) const
			{
				...
				std::set<std::string> setRequiredExtensions(DeviceExtensions.begin(), DeviceExtensions.end());
			...
			void Device::Impl::OnWindowResized(GLFWwindow* pWindow, int nWidth, int nHeight)
			{
			...
			VkCommandBuffer Device::GetCommandBuffer(size_t nIndex) const
			{
				return m_pImpl->GetCommandBuffer(nIndex);
			}
			...
			void Device::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t nWidth, uint32_t nHeight)
			{
				m_pImpl->CopyBufferToImage(buffer, image, nWidth, nHeight);
			}
			...
			void Device::CreateImage(uint32_t nWidth, uint32_t nHeight, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage* image_out, VkDeviceMemory* imageMemory_out)
			{
				return m_pImpl->CreateImage(nWidth, nHeight, format, tiling, usage, properties, image_out, imageMemory_out);
			}
