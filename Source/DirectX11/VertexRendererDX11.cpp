...
namespace eastengine
{
	namespace graphics
	{
		namespace dx11
		{
			...
			private:
				void CreateVertexShader(ID3D11Device* pDevice, ID3DBlob* pShaderBlob, const char* shaderPath, shader::VSType emVSType);
				void CreatePixelShader(ID3D11Device* pDevice, ID3DBlob* pShaderBlob, const char* shaderPath, const char* pFunctionName);
			...
			void VertexRenderer::Impl::Render(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, Camera* pCamera)
			{
			...
			void VertexRenderer::Impl::CreateVertexShader(ID3D11Device* pDevice, ID3DBlob* pShaderBlob, const char* shaderPath, shader::VSType emVSType)
			{
				...
				if (m_pInputLayout == nullptr)
				{
					...
					{
						LOG_ERROR("failed to create vertex shader : %s", functionName.c_str());
						return;
					}
				}
				else
			...
			void VertexRenderer::Impl::CreatePixelShader(ID3D11Device* pDevice, ID3DBlob* pShaderBlob, const char* shaderPath, const char* pFunctionName)
			{
				std::vector<D3D_SHADER_MACRO> vecMacros;
				vecMacros.push_back({ "DX11", "1" });
				vecMacros.push_back({ nullptr, nullptr });
				...
				{
					LOG_ERROR("failed to create pixel shader : %u", "VertexPS");
					return;
				}
			}
			...
			void VertexRenderer::Render(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, Camera* pCamera)
			{
