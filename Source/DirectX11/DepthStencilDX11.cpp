	...
	namespace graphics
	{
		namespace dx11
		{
			...
			std::unique_ptr<DepthStencil> DepthStencil::Create(const D3D11_TEXTURE2D_DESC* pDesc)
			{
				...
				{
					LOG_ERROR("DepthStencil's BindFlags is required include D3D11_BIND_DEPTH_STENCIL flag");
					return {};
				}
				...
				if (FAILED(hr))
				{
					...
					LOG_ERROR("failed to create DepthStencilView");
					return {};
				}
				...
				if ((desc.BindFlags & D3D11_BIND_SHADER_RESOURCE) == D3D11_BIND_SHADER_RESOURCE)
				{
					...
					if (FAILED(hr))
					{
						...
						LOG_ERROR("failed to create ShaderResourceView");
						return {};
					}
			...
			bool DepthStencil::GetDesc2D(D3D11_TEXTURE2D_DESC* pDesc) const
			{
				ID3D11Texture2D* pTexture2D = nullptr;
				m_pDepthStencilView->GetResource(reinterpret_cast<ID3D11Resource**>(&pTexture2D));
