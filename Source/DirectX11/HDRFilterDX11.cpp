	...
	namespace graphics
	{
		namespace dx11
		{
			...
			HDRFilter::Impl::Impl()
			{
				...
				if (util::CreateVertexShader(pDevice, pShaderBlob, macros, strShaderPath.c_str(), "VS", shader::VS_CompileVersion, &m_pVertexShader, "HDR_VS") == false)
				{
				...
				for (int i = 0; i < shader::ePS_Count; ++i)
				{
					...
					const char* strPSName = shader::GetHDRPSTypeToString(emPSType);
			...
			void HDRFilter::Impl::Apply(const RenderTarget* pSource, RenderTarget* pResult)
			{
				...
				ID3D11SamplerState* pSamplerLinearClamp = pDeviceInstance->GetSamplerState(EmSamplerState::eMinMagMipLinearClamp);
				...
				RenderTarget* pBloom = nullptr;
				{
					...
					pBloom = pDeviceInstance->GetRenderTarget(&desc, false);
					...
					{
						RenderTarget* pBlur = pDeviceInstance->GetRenderTarget(&desc, true);
