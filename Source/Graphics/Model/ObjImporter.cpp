...
namespace est
{
	namespace graphics
	{
		...
		bool ObjImporter::LoadModel(IModel* pModel, const wchar_t* fileName, const float scaleFactor, LOD emMaxLOD, const LODReductionRate* pLodReductionRate)
		{
			if (pModel == nullptr)
				return false;
			...
			file::Stream file;
			if (file.Open(fileName, file::eNone) == false)
				return false;

			if (loadModelData(file, scaleFactor) == false)
			{
				ClearData();
				return false;
			}
		...
		MaterialPtr ObjImporter::GetMaterial(const string::StringID& strName)
		{
			if (m_pMtlImporter == nullptr)
				return nullptr;
		...
		bool ObjImporter::loadModelData(file::Stream& file, const float scaleFactor)
		{
			m_objData.Clear();
			...
			while (file.Eof() == false)
			{
				...
				else if (temp == "mtllib")
				{
					...
					const std::wstring fileNameW = string::MultiToWide(fileName.c_str());
					if (m_pMtlImporter->Init(fileNameW.c_str(), file::GetFilePath(file.GetFilePath().c_str()).c_str()) == false)
					{
						LOG_WARNING(L"Cant Load Mtl File : %s", fileName.c_str());
					}
				}
				else if (temp == "g")
				{
					...
					{
						m_objData.GetCurGroupData().strGroupName = fileName;
					}
					else
					{
						ObjGroupData groupData;
						groupData.strGroupName = fileName;
				...
				else if (temp == "usemtl")
				{
					...
					{
						m_objData.GetCurGroupData().GetCurSubModel().strMtlName = fileName;
					}
					else
					{
						ObjSubModel subModel;
						subModel.strMtlName = fileName;
						...
						auto GetFace = [](const std::string& str) -> std::array<int, 3>
						{
							...
							for (size_t i = 0; i < str.size(); ++i)
							{
								if (str[i] == '/')
								{
									...
									{
										indexArray[index] = string::ToValue<int>(t.c_str());
									}
									else
									{
										indexArray[index] = 0;
								...
								if (i == str.size() - 1)
								{
									indexArray[index] = string::ToValue<int>(str.substr(nPrevPos).c_str());
								}
							}
		...
		bool ObjImporter::buildModel(IModel* pIModel, LOD emMaxLOD, const LODReductionRate* pLodReductionRate)
		{
			...
			for (auto& iter : m_objData.vecGroupData)
			{
				...
				VertexBufferPtr pVertexBuffer[eMaxLod] = { nullptr, };
				IndexBufferPtr pIndexBuffer[eMaxLod] = { nullptr, };

				std::vector<MaterialPtr> materials;
				std::vector<ModelSubset> modelSubsets[eMaxLod];
				...
				{
					uint32_t materialID = ModelSubset::eInvalidMaterialID;
					if (iter_sub.strMtlName.empty() == false)
					{
						MaterialPtr pMaterial = GetMaterial(iter_sub.strMtlName.c_str());
						if (pMaterial != nullptr)
						{
							bool bFind = false;
							for (auto& iter_mtrl : materials)
							{
								++materialID;
					...
					{
						size_t seed = 0;
						hash_combine(seed, vIdx);
						hash_combine(seed, tIdx);
						hash_combine(seed, nIdx);
					...
					for (size_t i = 0; i < nSize; ++i)
					{
						...
						modelSubset.startIndex = nStartIdx[i];
						modelSubset.materialID = materialID;
						modelSubset.indexCount = static_cast<uint32_t>(vecIndexOut.size());
						modelSubsets[i].push_back(modelSubset);
				...
				std::unique_ptr<ModelNodeStatic> pModelStatic = std::make_unique<ModelNodeStatic>(emMaxLod);
				...
				for (uint32_t i = 0; i < nSize; ++i)
				{
					...
					pModelStatic->AddModelSubsets(modelSubsets[i], emLOD);
				}
