...
namespace est
{
	namespace graphics
	{
		...
		bool MotionPlayer::Update(float elapsedTime)
		{
			...
			if (isPlaying == true)
			{
				...
				if (math::IsZero(GetBlendWeight()) == false)
				{
					...
					{
						playTime = endTime - m_playTime;
					}

					if (m_playBackInfo.isFreezeAtLastFrame == true && playTime >= endTime && m_isRecordedLastFrame == false)
					{
						playTime = endTime;
		...
		void MotionPlayer::Play(const MotionPtr& pMotion, const MotionPlaybackInfo* pPlayback, float blendTime)
		{
			if (pMotion == nullptr)
				return;
