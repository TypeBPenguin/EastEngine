...
#define BUILD_FLAVOR "Release"
		...
		public:
			...
			{
				const std::wstring wideString = string::MultiToWide(message);
				LOG_MESSAGE(L"%s", wideString.c_str());
			}
			virtual void LogWarning(const char* message) override
			{
				const std::wstring wideString = string::MultiToWide(message);
				LOG_WARNING(L"%s", wideString.c_str());
			}
			virtual void LogError(const char* message) override
			{
				const std::wstring wideString = string::MultiToWide(message);
				LOG_ERROR(L"%s", wideString.c_str());
			}
		};
		...
		public:
			...
			void SetUnitScale(const float scale) { m_fUnitScale = scale; }
		...
		private:
			...
			void AddKey(AnimationScanNode& asn, const AnimationScanNode* pParent, const FbxAMatrix& matFBXGlobal, float time);
			...
			void AddTextureParameter(ATG::ExportMaterial* pMaterial, const char* strParamName, uint32_t index, const char* fileName, DWORD nFlags);
			...
			struct SkinData
			{
				...
				byte* GetIndices(size_t index);
				float* GetWeights(size_t index);

				uint32_t GetBoneCount() const { return static_cast<uint32_t>(InfluenceNodes.size()); }

				void InsertWeight(size_t index, uint32_t nBoneIndex, float fBoneWeight);
			};
		...
		private:
			...
			tsl::robin_map<std::string, est::math::Matrix> m_umapMotionOffsetMarix;

			tsl::robin_map<std::string, fbxsdk::FbxMatrix> m_BindPoseMap;
		...
		bool FBXImport::Impl::LoadModel(Model* pModel, const wchar_t* filePath, float scale, bool isFlipZ)
		{
			if (pModel == nullptr || filePath == nullptr)
				return false;
			...
			g_CurrentOutputFileName = string::WideToMulti(file::GetFilePath(filePath)).c_str();

			ModelSettings();

			g_pScene->Settings().fExportScale = scale;
			g_pScene->Settings().bFlipZ = isFlipZ;

			g_pScene->Statistics().StartExport();
			g_pScene->Statistics().StartSceneParse();

			if (ImportFile(filePath) == false)
			{
				const std::string multiLog = string::WideToMulti(filePath);
				ExportLog::LogError("Could not load file \"%s\".", multiLog.c_str());
				Release();
				return false;
			}
		...
		bool FBXImport::Impl::LoadMotion(Motion* pMotion, const wchar_t* filePath, float scale)
		{
			if (pMotion == nullptr || filePath == nullptr)
				return false;
		...
		bool FBXImport::Impl::ImportFile(const wchar_t* fileName)
		{
			...
			const std::string multiLog = string::WideToMulti(fileName);
			ExportLog::LogMsg(1, "Loading FBX file \"%s\"...", multiLog.c_str());

			const std::string multiFileName = string::WideToMulti(fileName);
			int nFileFormat = -1;
			bool isSuccess = m_pImporter->Initialize(multiFileName.c_str(), nFileFormat, m_pSDKManager->GetIOSettings());
			...
			if (isSuccess == false)
			{
				ExportLog::LogError("Could not load FBX file \"%s\".", multiFileName.c_str());
				return false;
			}

			ExportLog::LogMsg(1, "FBX file \"%s\" was successfully loaded.", multiFileName.c_str());
			...
			if (g_pScene->Settings().bExportAnimations)
			{
				...
				{
					const std::string AnimName = file::GetFileNameWithoutExtension(multiFileName);
		...
		void FBXImport::Impl::AddKey(AnimationScanNode& asn, const AnimationScanNode* pParent, const FbxAMatrix& matFBXGlobal, float time)
		{
			...
			asn.pTrack->TransformTrack.AddKey(time,
		...
		void FBXImport::Impl::AddTextureParameter(ATG::ExportMaterial* pMaterial, const char* strParamName, uint32_t index, const char* fileName, DWORD dwFlags)
		{
			ExportMaterialParameter OutputParam;
			if (index == 0)
			{
			...
			{
				OutputParam.Name = string::Format("%s%u", strParamName, index).c_str();
			}

			ExportLog::LogMsg(4, "Material parameter \"%s\" = \"%s\"", OutputParam.Name.SafeString(), fileName);
			OutputParam.ValueString = fileName;
			OutputParam.ParamType = MPT_TEXTURE2D;
			OutputParam.bInstanceParam = true;
		...
		byte* FBXImport::Impl::SkinData::GetIndices(size_t index)
		{
			assert(index < nVertexCount);
			return pBoneIndices.get() + (index * nVertexStride);
		}

		float* FBXImport::Impl::SkinData::GetWeights(size_t index)
		{
			assert(index < nVertexCount);
			return pBoneWeights.get() + (index * nVertexStride);
		}

		void FBXImport::Impl::SkinData::InsertWeight(size_t index, uint32_t nBoneIndex, float fBoneWeight)
		{
			byte* pIndices = GetIndices(index);
			float* pWeights = GetWeights(index);
		...
		bool FBXImport::LoadModel(Model* pModel, const wchar_t* filePath, float scale, bool isFlipZ)
		{
			return m_pImpl->LoadModel(pModel, filePath, scale, isFlipZ);
		}

		bool FBXImport::LoadMotion(Motion* pMotion, const wchar_t* filePath, float scale)
		{
			return m_pImpl->LoadMotion(pMotion, filePath, scale);
		}
	}
}