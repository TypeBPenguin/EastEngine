...
namespace est
{
	namespace graphics
	{
		...
		void MotionRecorder::SetTransform(const string::StringID& boneName, const math::Transform& transform)
		{
			m_rmapMotionData[boneName] = transform;
		}

		const math::Transform* MotionRecorder::GetTransform(const string::StringID& boneName) const
		{
			auto iter = m_rmapMotionData.find(boneName);
