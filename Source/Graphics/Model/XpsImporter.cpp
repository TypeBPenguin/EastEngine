	...
	namespace graphics
	{
		namespace XPSImport
		{
			...
			bool LoadModel(Model* pModel, const wchar_t* filePath, const std::string* pStrDevideModels, size_t nKeywordCount)
			{
				...
				const std::wstring strPath = file::GetFilePath(filePath);
				const std::wstring strFileName = file::GetFileName(filePath);
				...
				{
					std::unique_ptr<ModelNodeSkinned> pSkinnedNode = std::make_unique<ModelNodeSkinned>();
					...
					if (isEnableMeshOptimize == true)
					{
						...
						for (uint32_t i = 0; i < nMeshCount; ++i)
						{
							...
							else
							{
								...
								meshOptimizer.numVertices += mesh.numVertices;
								meshOptimizer.numElements += mesh.numElements;
								meshOptimizer.name = mesh.name.c_str();
							}
						}

						modelSubsets.resize(umapMesh.size());
						materials.resize(umapMesh.size());
						...
						for (auto iter : umapMesh)
						{
							...
							modelSubsets[index].name = meshOptimizer.name;
							modelSubsets[index].startIndex = startIndex;
							modelSubsets[index].indexCount = meshOptimizer.numElements;
							modelSubsets[index].materialID = index;

							IMaterial::Data materialData;
							materialData.path = strPath;
							materialData.name = meshOptimizer.name;
							materialData.textureNameArray[IMaterial::eAlbedo] = std::get<0>(key).c_str();
							materialData.textureNameArray[IMaterial::eNormal] = std::get<1>(key).c_str();
							materials[index] = CreateMaterial(&materialData);
							...
							for (size_t i = 0; i < nSize; ++i)
							{
								...
								{
									vecIndices[startIndex + j] = nStartVertex + mesh.elements[j];
								}

								nStartVertex += mesh.numVertices;
								startIndex += mesh.numElements;
							}
						}
					}
					else
					{
						modelSubsets.resize(nMeshCount);
						materials.resize(nMeshCount);
						...
						for (uint32_t i = 0; i < nMeshCount; ++i)
						{
							...
							modelSubsets[i].name = mesh.name.c_str();
							modelSubsets[i].startIndex = startIndex;
							modelSubsets[i].indexCount = mesh.numElements;
							modelSubsets[i].materialID = i;
							...
							if (mesh.numTextures == 1)
							{
								materialData.textureNameArray[IMaterial::eAlbedo] = file::GetFileName(mesh.textures[0].filename).c_str();
							}
							else if (mesh.numTextures == 3)
							{
								materialData.textureNameArray[IMaterial::eAlbedo] = file::GetFileName(mesh.textures[0].filename).c_str();
								materialData.textureNameArray[IMaterial::eNormal] = file::GetFileName(mesh.textures[1].filename).c_str();
							}
							else if (mesh.numTextures == 4)
							{
								materialData.textureNameArray[IMaterial::eAlbedo] = file::GetFileName(mesh.textures[0].filename).c_str();
								materialData.textureNameArray[IMaterial::eNormal] = file::GetFileName(mesh.textures[2].filename).c_str();
							}

							materials[i] = CreateMaterial(&materialData);
					...
					VertexBufferPtr pVertexBuffer = CreateVertexBuffer(reinterpret_cast<uint8_t*>(vecVertices.data()), static_cast<uint32_t>(vecVertices.size()), sizeof(VertexPosTexNorWeiIdx), true);
					pSkinnedNode->SetVertexBuffer(pVertexBuffer);

					IndexBufferPtr pIndexBuffer = CreateIndexBuffer(reinterpret_cast<uint8_t*>(vecIndices.data()), static_cast<uint32_t>(vecIndices.size()), sizeof(uint32_t), true);
					pSkinnedNode->SetIndexBuffer(pIndexBuffer);

					pSkinnedNode->AddModelSubsets(modelSubsets);

					pSkinnedNode->AddMaterialArray(materials.data(), materials.size());
					...
					const string::StringID nodeName = pSkinnedNode->GetName();
					pModel->AddNode(std::move(pSkinnedNode), nodeName);
				}
			...
			bool LoadMotion(Motion* pMotion, const wchar_t* filePath)
			{
				...
				while (file.Eof() == false)
				{
					...
					if (strTemp.empty() == false)
					{
						...
						name = strTemp.substr(0, nFindPos).c_str();
						...
						pMotion->AddBoneKeyframes(name, std::move(vecKeyframes));
					}
				}
