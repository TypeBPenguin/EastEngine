	...
	namespace graphics
	{
		namespace dx11
		{
			...
			GaussianBlur::Impl::Impl()
			{
				...
				if (util::CreateVertexShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "VS", shader::VS_CompileVersion, &m_pVertexShader, "GaussianBlur_VS") == false)
				{
					throw_line("failed to create GaussianBlur_VS");
				}

				if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "GaussianBlurH_PS", shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_GaussianBlurH], "GaussianBlurH_PS") == false)
				{
					throw_line("failed to create GaussianBlurH_PS");
				}

				if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "GaussianBlurV_PS", shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_GaussianBlurV], "GaussianBlurV_PS") == false)
				{
					throw_line("failed to create GaussianBlurV_PS");
				}

				if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "GaussianDepthBlurH_PS", shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_GaussianDepthBlurH], "GaussianDepthBlurH_PS") == false)
				{
					throw_line("failed to create GaussianDepthBlurH_PS");
				}

				if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "GaussianDepthBlurV_PS", shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_GaussianDepthBlurV], "GaussianDepthBlurV_PS") == false)
				{
			...
			void GaussianBlur::Impl::Apply(const RenderTarget* pSource, RenderTarget* pResult, float fSigma)
			{
				...
				RenderTarget* pGaussianBlur = pDeviceInstance->GetRenderTarget(&desc);
			...
			void GaussianBlur::Impl::SetRenderState(Device* pDeviceInstance, ID3D11DeviceContext* pDeviceContext)
			{
				...
				ID3D11SamplerState* pSampler = pDeviceInstance->GetSamplerState(SamplerState::eMinMagMipPointWrap);
