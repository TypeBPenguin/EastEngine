	...
	namespace graphics
	{
		namespace dx11
		{
			...
			SSS::Impl::Impl()
			{
				...
				if (util::CreateVertexShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "SSSSBlurVS", shader::VS_CompileVersion, &m_pVertexShader, "SSS_VS") == false)
				{
				...
				for (int i = 0; i < shader::ePS_Count; ++i)
				{
					...
					const char* psName = shader::GetSSSPSTypeToString(emPSType);
			...
			void SSS::Impl::Apply(const RenderTarget* pSource, const DepthStencil* pDepth, RenderTarget* pResult)
			{
				...
				ID3D11SamplerState* pSamplerLinearPointClamp = pDeviceInstance->GetSamplerState(SamplerState::eMinMagLinearMipPointClamp);
				...
				shader::SetSSSContents(pDeviceContext, &m_sssContents, config.width);
