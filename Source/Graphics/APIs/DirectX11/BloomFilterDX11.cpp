	...
	namespace graphics
	{
		namespace dx11
		{
			...
			BloomFilter::Impl::Impl()
			{
				...
				if (util::CreateVertexShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "VS", shader::VS_CompileVersion, &m_pVertexShader, "BloomFilter_VS") == false)
				{
				...
				for (int i = 0; i < shader::ePS_Count; ++i)
				{
					...
					const char* psName = shader::GetBloomFilterPSTypeToString(emPSType);
			...
			RenderTarget* BloomFilter::Impl::Sampling(Device* pDevice, ID3D11DeviceContext* pDeviceContext, const Options::BloomFilterConfig& bloomFilterConfig, shader::PSType emPSType, bool isResult, uint32_t nWidth, uint32_t nHeight, int nPass, const math::float2& f2InverseResolution, RenderTarget* pSource, RenderTarget* pResult)
			{
				...
				est::graphics::dx11::util::DXProfiler profiler(wstrDebugName.c_str());
				...
				if (m_nDownsamplePasses > nPass)
				{
					...
					if (pResult == nullptr)
					{
						...
						pResult = pDevice->GetRenderTarget(&desc);
					}
