	...
	namespace graphics
	{
		namespace dx11
		{
			...
			DownScale::Impl::Impl()
			{
				...
				shaderPath.append(L"Fx\\PostProcessing\\DownScale\\DownScale.hlsl");
...

				{
					...
					if (util::CreateVertexShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "VS", shader::VS_CompileVersion, &m_pVertexShader, "DownScaleVS") == false)
					{
						throw_line("failed to create DownScaleVS");
					}

					if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), GetDownScalePSTypeString(shader::ePS_Downscale4), shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_Downscale4], GetDownScalePSTypeString(shader::ePS_Downscale4)) == false)
					{
						throw_line("failed to create DownscalePS");
					}

					if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), GetDownScalePSTypeString(shader::ePS_DownscaleHW), shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_DownscaleHW], GetDownScalePSTypeString(shader::ePS_DownscaleHW)) == false)
					{
						throw_line("failed to create HWScalePS");
					}

					if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), GetDownScalePSTypeString(shader::ePS_Downscale4Luminance), shader::PS_CompileVersion, &m_pPixelShaders[shader::ePS_Downscale4Luminance], GetDownScalePSTypeString(shader::ePS_Downscale4Luminance)) == false)
					{
			...
			void DownScale::Impl::Apply16SW(const RenderTarget* pSource, RenderTarget* pResult)
			{
				...
				RenderTarget* pDownscale = pDeviceInstance->GetRenderTarget(&desc);
