	...
	namespace graphics
	{
		namespace dx11
		{
			...
			DepthOfField::Impl::Impl()
			{
				...
				if (util::CreateVertexShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "VS", shader::VS_CompileVersion, &m_pVertexShader, "DepthOfField_VS") == false)
				{
					throw_line("failed to create DepthOfField_VS");
				}

				if (util::CreatePixelShader(pDevice, pShaderBlob, macros, shaderPath.c_str(), "DofDiscPS", shader::PS_CompileVersion, &m_pPixelShader, "DofDiscPS") == false)
				{
