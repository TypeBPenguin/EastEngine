	...
	namespace graphics
	{
		...
		namespace vulkan
		{
			namespace deferredshader
			{
				...
				{
					static std::wstring strVertShaderPath;
					if (strVertShaderPath.empty() == false)
						return strVertShaderPath.c_str();

					strVertShaderPath = file::GetEngineDataPath();
					strVertShaderPath.append(L"Fx\\FullScnreeQuad.vert");
				...
				{
					static std::wstring strFragShaderPath;
					if (strFragShaderPath.empty() == false)
						return strFragShaderPath.c_str();

					strFragShaderPath = file::GetEngineDataPath();
					strFragShaderPath.append(L"Fx\\Model\\Deferred.frag");
			...
			void DeferredRenderer::Impl::Render(Camera* pCamera)
			{
				...
				{
					deferredshader::CommonContents* pCommonContents = m_vecCommonContentsBuffers[frameIndex].Cast(0);
					pCommonContents->matInvView = pCamera->GetViewMatrix().Invert();
					pCommonContents->matInvProj = pCamera->GetProjectionMatrix().Invert();
					...
					memoryRange.memory = m_vecCommonContentsBuffers[frameIndex].uniformBufferMemory;
					memoryRange.size = m_vecCommonContentsBuffers[frameIndex].Size();
					vkFlushMappedMemoryRanges(device, 1, &memoryRange);
				}

				{
					deferredshader::LightContents* pLightContents = m_vecLightContentsBuffers[frameIndex].Cast(0);

					const DirectionalLightData* pDirectionalLightData = nullptr;
					pLightManager->GetDirectionalLightRenderData(&pDirectionalLightData, &pLightContents->nDirectionalLightCount);
					...
					pLightManager->GetPointLightRenderData(&pPointLightData, &pLightContents->nPointLightCount);
					...
					pLightManager->GetSpotLightRenderData(&pSpotLightData, &pLightContents->nSpotLightCount);
					...
					VkMappedMemoryRange memoryRange{};
					memoryRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
					memoryRange.memory = m_vecLightContentsBuffers[frameIndex].uniformBufferMemory;
					memoryRange.size = m_vecLightContentsBuffers[frameIndex].Size();
					vkFlushMappedMemoryRanges(device, 1, &memoryRange);
				}
				...
				auto SetImageInfo = [&](VkImageView imageView, VkSampler sampler, deferredshader::Binding emBinding)
				{
					...
					descriptorWrite.dstSet = m_vecDescriptorSets[frameIndex];
				...
				SetImageInfo(pGBuffer->GetRenderTarget(GBufferType::eNormals)->GetImageView(), samplerPointClamp, deferredshader::eBind_TexNormal);
				SetImageInfo(pGBuffer->GetRenderTarget(GBufferType::eColors)->GetImageView(), samplerPointClamp, deferredshader::eBind_TexAlbedoSpecular);
				SetImageInfo(pGBuffer->GetRenderTarget(GBufferType::eDisneyBRDF)->GetImageView(), samplerPointClamp, deferredshader::eBind_TexDisneyBRDF);
				...
				{
					m_vecDescriptorSets[frameIndex],
					m_vecLightContentsBuffers[frameIndex].descriptorSet,
					m_vecCommonContentsBuffers[frameIndex].descriptorSet
				};
