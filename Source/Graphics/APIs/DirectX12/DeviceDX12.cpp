	...
	namespace graphics
	{
		namespace dx12
		{
			...
			private:
				...
				std::array<uint32_t, EmSamplerState::TypeCount> m_samplerStates{ 0 };
			};
				...
				// UpdateOptions
				{
						...
						prevOptions.motionBlurConfig.IsVelocityMotionBlur() != options.motionBlurConfig.IsVelocityMotionBlur())
					{
						if (options.OnMotionBlur == true && options.motionBlurConfig.IsVelocityMotionBlur() == true)
						{
							...
							{
								m_pGBuffers[i]->Resize(GBufferType::eVelocity, m_n2ScreenSize.x, m_n2ScreenSize.y);
							}
						}
						else
						{
			...
			void Device::Impl::Initialize(uint32_t width, uint32_t height, bool isFullScreen, const string::StringID& applicationTitle, const string::StringID& applicationName)
			{
				...
				AddMessageHandler(sid::DeviceDX12, [&](HWND hWnd, uint32_t nMsg, WPARAM wParam, LPARAM lParam)
				{
			...
			void Device::Impl::Release()
			{
				...
				RemoveMessageHandler(sid::DeviceDX12);
			...
			{
				ID3D12CommandList** ppCommandLists = static_cast<ID3D12CommandList**>(_malloca(sizeof(ID3D12CommandList*) * nCount));
			...
			void Device::Impl::InitializeD3D()
			{
...

				{
					...
					m_nNullTextureIndex = srvAlloc.index;
			...
			void Device::Impl::InitializeSampler()
			{
				for (uint32_t i = 0; i < EmSamplerState::TypeCount; ++i)
				{
					...
					m_samplerStates[emSamplerState] = samplerAlloc.index;
				}
			}
