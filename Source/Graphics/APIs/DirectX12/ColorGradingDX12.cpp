		...
		namespace dx12
		{
			...
			void ColorGrading::Impl::Apply(Camera* pCamera, RenderTarget* pSource, RenderTarget* pResult)
			{
...

				{
					...
					shader::ColorGradingContents* pColorGradingContents = m_colorGradingContents.Cast(frameIndex);
			...
			ID3D12RootSignature* ColorGrading::Impl::CreateRootSignature(ID3D12Device* pDevice)
			{
				...
				{
					util::GetStaticSamplerDesc(SamplerState::eMinMagMipLinearWrap, 0, 100, D3D12_SHADER_VISIBILITY_PIXEL),
				};
			...
			void ColorGrading::Impl::CreatePipelineState(ID3D12Device* pDevice, ID3DBlob* pShaderBlob, const wchar_t* shaderPath)
			{
