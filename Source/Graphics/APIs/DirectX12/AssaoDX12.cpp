	...
	namespace graphics
	{
		namespace dx12
		{
			...
			struct D3D12Texture2D
			{
				...
				void Reset();
				bool ReCreateIfNeeded(ID3D12Device* pDevice, const math::uint2& size, DXGI_FORMAT format, float& inoutTotalSizeSum, uint16_t mipLevels, uint16_t arraySize, const math::Color& clearColor, bool supportUAVs, const wchar_t* debugName);
			...
			bool D3D12Texture2D::ReCreateIfNeeded(ID3D12Device* pDevice, const math::uint2& size, DXGI_FORMAT format, float& inoutTotalSizeSum, uint16_t mipLevels, uint16_t arraySize, const math::Color& clearColor, bool supportUAVs, const wchar_t* debugName)
			{
				...
				else
				{
					...
					pDevice->CreateShaderResourceView(Resource, &srvDesc, srvAlloc.cpuHandle);
					...
					if (arraySize == 1)
					{
						...
						RTVIndex = rtvAlloc.index;

						D3D12_RENDER_TARGET_VIEW_DESC rtvDesc{};
						rtvDesc.Format = format;
						rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;

						pDevice->CreateRenderTargetView(Resource, &rtvDesc, rtvAlloc.cpuHandle);
					}
					...
					if (supportUAVs == true)
					{
						...
						UAVIndex = uavAlloc.index;

						D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc{};
						uavDesc.Format = format;
						uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;

						pDevice->CreateUnorderedAccessView(Resource, nullptr, &uavDesc, uavAlloc.cpuHandle);
					}
			...
			struct AssaoInputsDX12 : public IAssaoInputs
			{
				...
				uint32_t DepthSRV{ eInvalidDescriptorIndex };
				...
				uint32_t NormalSRV{ eInvalidDescriptorIndex };
				...
				uint32_t OverrideOutputRTV{ eInvalidDescriptorIndex };

				uint32_t frameIndex{ eFrameBufferCount };
			...
			private:
				...
				shader::ASSAOSRVContents* AllocateSRVContents(uint32_t frameIndex)
				{
					assert(m_nSrvBufferIndex < eMaxSRVBufferCount);
					shader::ASSAOSRVContents* pSRV = m_srvContentsBuffer.Cast(frameIndex, m_nSrvBufferIndex);
					m_srvContentsBufferGPUAddress = m_srvContentsBuffer.GPUAddress(frameIndex, m_nSrvBufferIndex);
					m_nSrvBufferIndex++;

					return pSRV;
				}

				void ResetSRVContents(uint32_t frameIndex)
				{
					m_nSrvBufferIndex = 0;
					m_srvContentsBufferGPUAddress = m_srvContentsBuffer.GPUAddress(frameIndex, 0);
				}

				shader::ASSAOConstants* AllocateConstantBuffer(uint32_t frameIndex)
				{
					assert(m_nConstantsBufferIndex < eMaxConstantsBufferCount);
					shader::ASSAOConstants* pConstantBuffer = m_constantsBuffer.Cast(frameIndex, m_nConstantsBufferIndex);
					m_constantsBufferGPUAddress = m_constantsBuffer.GPUAddress(frameIndex, m_nConstantsBufferIndex);
					m_nConstantsBufferIndex++;

					return pConstantBuffer;
				}

				void ResetConstantBuffer(uint32_t frameIndex)
				{
					m_nConstantsBufferIndex = 0;
					m_constantsBufferGPUAddress = m_constantsBuffer.GPUAddress(frameIndex, 0);
				}
			...
			void Assao::Impl::Draw(const Options::AssaoConfig& config, const IAssaoInputs* _inputs)
			{
				...
				assert(inputs->DepthSRV != eInvalidDescriptorIndex);
				assert(inputs->NormalSRV != eInvalidDescriptorIndex);
				assert(inputs->OverrideOutputRTV != eInvalidDescriptorIndex);
				assert(inputs->frameIndex != -1);
...
#endif
				ResetSRVContents(inputs->frameIndex);
				ResetConstantBuffer(inputs->frameIndex);
...

				{
					...
					{
						if (m_normals.RTVIndex != eInvalidDescriptorIndex)
						{
					...
					if (config.QualityLevel == 3)
					{
						...
						// Generate importance map
						{
...

							{
								...
								shader::ASSAOSRVContents* pSRV = AllocateSRVContents(inputs->frameIndex);
							...
							// postprocess B
							{
								...
								D3D12_CPU_DESCRIPTOR_HANDLE cpuStartHandle = inputs->pDescriptorHeapUAV->GetStartCPUHandle(inputs->frameIndex);
					...
					if (inputs->OverrideOutputRTV != eInvalidDescriptorIndex)
					{
			...
			bool Assao::Impl::InitializeDX()
			{
				...
				for (int i = 0; i < shader::ePS_Count; ++i)
				{
					...
					CreatePipelineState(m_pDevice, pShaderBlob, shaderPath.c_str(), emPSType);
				}
				...
				// load counter stuff, only needed for adaptive quality (quality level 3)
				{
...

					{
						DescriptorHeap* pDescriptorHeapSRV = Device::GetInstance()->GetSRVDescriptorHeap();
						PersistentDescriptorAlloc srvAlloc = pDescriptorHeapSRV->AllocatePersistent();
						m_nLoadCounterDescriptorSRVIndex = srvAlloc.index;
						...
						m_pDevice->CreateShaderResourceView(m_loadCounter, &srvDesc, srvAlloc.cpuHandle);
					}

					{
						DescriptorHeap* pDescriptorHeapUAV = Device::GetInstance()->GetUAVDescriptorHeap();
						PersistentDescriptorAlloc uavAlloc = pDescriptorHeapUAV->AllocatePersistent();
						m_nLoadCounterDescriptorUAVIndex = uavAlloc.index;

						D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc{};
						uavDesc.Format = DXGI_FORMAT_R32_UINT;
						uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE1D;

						m_pDevice->CreateUnorderedAccessView(m_loadCounter, nullptr, &uavDesc, uavAlloc.cpuHandle);
					}
				}
#endif
			...
			void Assao::Impl::CleanupDX()
			{
				...
				if (m_nLoadCounterDescriptorSRVIndex != eInvalidDescriptorIndex)
				{
					pDescriptorHeapSRV->FreePersistent(m_nLoadCounterDescriptorSRVIndex);
					m_nLoadCounterDescriptorSRVIndex = 0;
				}

				if (m_nLoadCounterDescriptorUAVIndex != eInvalidDescriptorIndex)
				{
					pDescriptorHeapUAV->FreePersistent(m_nLoadCounterDescriptorUAVIndex);
					m_nLoadCounterDescriptorUAVIndex = eInvalidDescriptorIndex;
				}
			...
			void Assao::Impl::UpdateTextures(const AssaoInputsDX12* inputs)
			{
				...
				if (inputs->NormalSRV != eInvalidDescriptorIndex)
				{
					if (m_normals.SRVIndex != eInvalidDescriptorIndex)
					{
						needsUpdate = true;
						m_normals.Reset();
					}
				...
				for (int i = 0; i < 4; i++)
				{
					const std::wstring debugName = string::Format(L"HalfDepth_%d", i);
					if (m_halfDepths[i].ReCreateIfNeeded(m_pDevice, m_halfSize, m_formats.DepthBufferViewspaceLinear, totalSizeInMB, SSAO_DEPTH_MIP_LEVELS, 1, math::Color::Transparent, false, debugName.c_str()))
					{
				...
				m_pingPongHalfResultA.ReCreateIfNeeded(m_pDevice, m_halfSize, m_formats.AOResult, totalSizeInMB, 1, 1, math::Color::White, false, L"PongHalfResultA");
				m_pingPongHalfResultB.ReCreateIfNeeded(m_pDevice, m_halfSize, m_formats.AOResult, totalSizeInMB, 1, 1, math::Color::Transparent, false, L"PongHalfResultB");
				m_finalResults.ReCreateIfNeeded(m_pDevice, m_halfSize, m_formats.AOResult, totalSizeInMB, 1, 4, math::Color::Transparent, false, L"FinalResult");
#ifdef INTEL_SSAO_ENABLE_ADAPTIVE_QUALITY
				m_importanceMap.ReCreateIfNeeded(m_pDevice, m_quarterSize, m_formats.ImportanceMap, totalSizeInMB, 1, 1, math::Color::Transparent, false, L"ImportanceMap");
				m_importanceMapPong.ReCreateIfNeeded(m_pDevice, m_quarterSize, m_formats.ImportanceMap, totalSizeInMB, 1, 1, math::Color::Transparent, false, L"ImportanceMapPong");
#endif
				...
				if (inputs->NormalSRV == eInvalidDescriptorIndex)
				{
					m_normals.ReCreateIfNeeded(m_pDevice, m_size, m_formats.Normals, totalSizeInMB, 1, 1, math::Color::Transparent, true, L"Normals");
				}
			...
			{
				const uint32_t frameIndex = inputs->frameIndex;

				const bool generateNormals = inputs->NormalSRV == eInvalidDescriptorIndex;

				// update constants
				shader::ASSAOConstants& consts = *AllocateConstantBuffer(frameIndex);
				{
			...
			void Assao::Impl::FullscreenPassDraw(const AssaoInputsDX12* inputs, shader::PSType emPSType)
			{
				...
				{
					inputs->pDescriptorHeapSRV->GetHeap(),
				};
				...
				inputs->pCommandList->ExecuteBundle(m_pipelineStates[emPSType].pBundles[inputs->frameIndex]);
			}
			...
			void Assao::Impl::PrepareDepths(const Options::AssaoConfig& config, const AssaoInputsDX12* inputs)
			{
				...
				else
				{
					...
					//if (config.QualityLevel < 0)
					//{
					//	shader::ASSAOSRVContents* pSRV = m_srvContentsBuffer.Cast(inputs->frameIndex, shader::ePS_PrepareDepthsAndNormalsHalf);
					//	pSRV->nTexDepthSourceIndex = inputs->DepthSRV;
					//
					//	pCommandList->OMSetRenderTargets(_countof(twoDepths), twoDepths, FALSE, nullptr);
					//
					...
					//else
					//{
					//	shader::ASSAOSRVContents* pSRV = m_srvContentsBuffer.Cast(inputs->frameIndex, shader::ePS_PrepareDepthsAndNormals);
					//	pSRV->nTexDepthSourceIndex = inputs->DepthSRV;
					//
					//	pCommandList->OMSetRenderTargets(_countof(fourDepths), fourDepths, FALSE, nullptr);
					//
			...
			{
				const int normalmapSRV = (inputs->NormalSRV == eInvalidDescriptorIndex) ? (m_normals.SRVIndex) : (inputs->NormalSRV);
			...
			void Assao::Impl::CreatePipelineState(ID3D12Device* pDevice, ID3DBlob* pShaderBlob, const wchar_t* shaderPath, shader::PSType emPSType)
			{
				...
				if (pShaderBlob != nullptr)
				{
					...
					{
						const bool isSuccess = util::CompileShader(pShaderBlob, macros, shaderPath, "VSMain", shader::VS_CompileVersion, &psoCache.pVSBlob);
						if (isSuccess == false)
						{
					...
					{
						const bool isSuccess = util::CompileShader(pShaderBlob, macros, shaderPath, shader::GetASSAOPSTypeString(emPSType), shader::PS_CompileVersion, &psoCache.pPSBlob);
						if (isSuccess == false)
						{
			...
			void Assao::Apply(Camera* pCamera, const RenderTarget* pNormalMap, const DepthStencil* pDepth, RenderTarget* pResult)
			{
				...
				inputs.frameIndex = pDeviceInstance->GetFrameIndex();
