	...
	namespace graphics
	{
		namespace dx12
		{
			...
			MapResult Uploader::AcquireTempBufferMem(uint64_t nSize, uint64_t nAlignment)
			{
				...
				MapResult result;
				result.pCPUAddress = m_pTempFrameCPUMem[frameIndex] + nOffset;
				result.nGPUAddress = m_nTempFrameGPUMem[frameIndex] + nOffset;
				result.nResourceOffset = nOffset;
				result.pResource = m_pTempFrameBuffers[frameIndex];
			...
			void Uploader::ClearFinishedUploads(uint64_t nFlushCount)
			{
				...
				{
					const uint64_t index = (nStart + i) % m_nSubmissionCount;
					UploadSubmission& subMission = m_uploadSubmissions[index];
