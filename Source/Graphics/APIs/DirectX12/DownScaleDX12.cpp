	...
	namespace graphics
	{
		namespace dx12
		{
			...
			void DownScale::Impl::Apply16SW(const RenderTarget* pSource, RenderTarget* pResult)
			{
				...
				RenderTarget* pDownscale = pDeviceInstance->GetRenderTarget(&desc, math::Color::Transparent);
			...
			void DownScale::Impl::CreatePipelineState(ID3D12Device* pDevice, ID3DBlob* pShaderBlob, const wchar_t* shaderPath, shader::PSType emPSType)
			{
				...
				if (pShaderBlob != nullptr)
				{
					...
					{
						const bool isSuccess = util::CompileShader(pShaderBlob, macros, shaderPath, GetDownScalePSTypeString(emPSType), shader::PS_CompileVersion, &psoCache.pPSBlob);
						if (isSuccess == false)
						{
			...
			void DownScale::Impl::ApplyDownScale(Device* pDeviceInstance, shader::PSType emPSType, const RenderTarget* pSource, RenderTarget* pResult)
			{
...

				{
					...
					shader::DownScaleContents* pDownScaleContents = m_downScaleContents.Cast(frameIndex);
