	...
	namespace graphics
	{
		namespace dx12
		{
			...
			public:
				...
				static std::unique_ptr<RenderTarget> Create(const D3D12_RESOURCE_DESC* pResourceDesc, const math::Color& clearColor, D3D12_RESOURCE_STATES resourceState, uint32_t nMipSlice = 0, uint32_t nFirstArraySlice = 0, uint32_t nArraySize = -1);
...
{
	template <>
	struct hash<est::graphics::dx12::RenderTarget::Key>
	{
