...
namespace est
{
	namespace graphics
	{
		...
		private:
			void WriteBMP(const wchar_t *filename, const unsigned char *data, int w, int h);
		...
		void OcclusionCulling::Impl::Update(Camera* pCamera)
		{
			if (IsEnable() == false)
				return;
		...
		void OcclusionCulling::Impl::RenderTriangles(const math::Matrix& worldMatrix, const VertexPos* pVertices, const uint32_t* pIndices, int indexCount)
		{
			...
			{
				LOG_WARNING(L"falied to RenderTriangles, OcclusionCulling thread is suspend mode now");
				return;
			}
		...
		void OcclusionCulling::Impl::WriteBMP(const wchar_t *filename, const unsigned char *data, int w, int h)
		{
			if (IsEnable() == false)
				return;
			...
			{
				LOG_ERROR(L"Error trying to save to %s", filename);
			}
		}
		...
		void OcclusionCulling::Impl::Write(const wchar_t* path)
		{
			if (IsEnable() == false)
				return;
		...
		void OcclusionCulling::Update(Camera* pCamera)
		{
		...
		void OcclusionCulling::Write(const wchar_t* path)
		{
			m_pImpl->Write(path);
		}
	}
