...
namespace est
{
	namespace graphics
	{
		...
		void Shadow::Update(const math::float3& position, const math::float3& direction, float intensity, float angle)
		{
			...
			m_viewMatrix = math::Matrix::CreateLookAt(viewPosition, position, math::float3::Up);

			m_projectionMatrix = math::Matrix::CreatePerspectiveFieldOfView(fov, 1.f, 0.01f, depth);
