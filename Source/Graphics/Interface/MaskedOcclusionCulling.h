...
public:
	...
	struct VertexLayout
	{
		...
		int mStride{ 0 };      //!< byte stride between vertices
		int mOffsetY{ 0 };     //!< byte offset from X to Y coordinate
		union
		{
	...
	struct ScissorRect
	{
		...
		int mMaxX{ 0 }; //!< Screen space X coordinate for right side of scissor rect, <B>non</B> inclusive and must be a multiple of 32
		int mMaxY{ 0 }; //!< Screen space Y coordinate for top side of scissor rect, <B>non</B> inclusive and must be a multiple of 8
	};
