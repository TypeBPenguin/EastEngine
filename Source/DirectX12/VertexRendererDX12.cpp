	...
	namespace graphics
	{
		namespace dx12
		{
			...
			private:
				...
				void CreatePipelineState(ID3D12Device* pDevice, ID3DBlob* pShaderBlob, const char* strShaderPath, shader::VSType emVSType);

				shader::SingleContents* AllocateSingleContents(uint32_t nFrameIndex)
				{
					assert(m_singleContentsIndex < shader::eMaxJobCount);
					shader::SingleContents* pBuffer = m_singleContents.Cast(nFrameIndex, m_singleContentsIndex);
					m_singleContentsGPUAddress = m_singleContents.GPUAddress(nFrameIndex, m_singleContentsIndex);
					++m_singleContentsIndex;

					return pBuffer;
				}

				void ReseSingleContents(uint32_t nFrameIndex)
				{
					m_singleContentsIndex = 0;
					m_singleContentsGPUAddress = m_singleContents.GPUAddress(nFrameIndex, 0);
				}

				shader::InstanceContents* AllocateInstanceContents(uint32_t nFrameIndex)
				{
					assert(m_instanceContentsIndex < shader::eMaxInstancingJobCount);
					shader::InstanceContents* pBuffer = m_instanceContents.Cast(nFrameIndex, m_instanceContentsIndex);
					m_instanceContentsGPUAddress = m_instanceContents.GPUAddress(nFrameIndex, m_instanceContentsIndex);
					++m_instanceContentsIndex;

					return pBuffer;
				}

				void ResetInstanceContents(uint32_t nFrameIndex)
				{
					m_instanceContentsIndex = 0;
					m_instanceContentsGPUAddress = m_instanceContents.GPUAddress(nFrameIndex, 0);
				}
			...
			{
				std::string shaderPath = file::GetPath(file::eFx);
				shaderPath.append("Vertex\\Vertex.hlsl");

				ID3DBlob* pShaderBlob = nullptr;
				if (FAILED(D3DReadFileToBlob(string::MultiToWide(shaderPath).c_str(), &pShaderBlob)))
				{
			...
			void VertexRenderer::Impl::Render(Camera* pCamera)
			{
				...
				ReseSingleContents(nFrameIndex);
				ResetInstanceContents(nFrameIndex);
				...
				for (auto& iter : m_umapJobVertexBatchs)
				{
					...
					const VertexBuffer* pVertexBuffer = static_cast<const VertexBuffer*>(batch.job.pVertexBuffer);
					const IndexBuffer* pIndexBuffer = static_cast<const IndexBuffer*>(batch.job.pIndexBuffer);

					const size_t size = batch.instanceData.size();
					if (size == 1)
					{
						shader::SingleContents* pSingleContents = AllocateSingleContents(nFrameIndex);
				...
				for (auto& iter : m_umapJobVertexBatchs)
				{
					...
					if (size > 1)
					{
						...
						for (size_t i = 0; i < loopCount; ++i)
						{
							...
							shader::InstanceContents* pInstanceContents = AllocateInstanceContents(nFrameIndex);
			...
			void VertexRenderer::Impl::CreatePipelineState(ID3D12Device* pDevice, ID3DBlob* pShaderBlob, const char* shaderPath, shader::VSType emVSType)
			{
			...
			void VertexRenderer::Render(Camera* pCamera)
			{
