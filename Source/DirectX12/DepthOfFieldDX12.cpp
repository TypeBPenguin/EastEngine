	...
	namespace graphics
	{
		namespace dx12
		{
			...
			DepthOfField::Impl::Impl()
			{
				...
				{
					if (util::CreateConstantBuffer(pDevice, m_depthOfFieldContents.AlignedSize(), &m_depthOfFieldContents.pUploadHeaps[i], L"DownScaleContents") == false)
					{
						throw_line("failed to create constant buffer, DownScaleContents");
					}
				}

				m_depthOfFieldContents.Initialize(m_depthOfFieldContents.AlignedSize());
			...
			DepthOfField::Impl::~Impl()
			{
				...
				for (int i = 0; i < eFrameBufferCount; ++i)
				{
					SafeRelease(m_depthOfFieldContents.pUploadHeaps[i]);
				}
			...
			ID3D12RootSignature* DepthOfField::Impl::CreateRootSignature(ID3D12Device* pDevice)
			{
				...
				lightContentsParameter.Descriptor.ShaderRegister = shader::eCB_DepthOfFieldContents;
