	...
	namespace graphics
	{
		namespace dx12
		{
			...
			struct ConstantBuffer
			{
				...
				static constexpr size_t AlignedSize() noexcept { return util::AlignTo(sizeof(T), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT); }
			...
			namespace util
			{
				inline uint64_t AlignTo(uint64_t nNum, uint64_t nAlignment) { return ((nNum + nAlignment - 1) / nAlignment) * nAlignment; }
