...
float3 CalcWorldSpacePosFromDepth(in float depth, in float2 uv, in float4x4 matInvViewProj)
{
	float x = uv.x * 2.f - 1.f;
	float y = (1.f - uv.y) * 2.f - 1.f;
	float z = depth;

	float4 screenSpacePosition = float4(x, y, z, 1.f);
	float4 viewSpacePosition = mul(screenSpacePosition, matInvViewProj);
	return viewSpacePosition.xyz /= viewSpacePosition.w;
}

float3 CalcWorldSpacePosFromDepth(in float depth, in float2 uv, in float4x4 matInvView, in float4x4 matInvProj)
{
	float4x4 matInvViewProj = matInvView * matInvProj;
	return CalcWorldSpacePosFromDepth(depth, uv, matInvViewProj);
}
...
float3 CalcWorldSpacePosFromDepth(in float depth, in float2 uv, in float4x4 matInvView, in float4x4 matInvProj, out float4 viewSpacePosition_out)
{
	...
	float4 screenSpacePosition = float4(x, y, z, 1.f);
	viewSpacePosition_out = mul(screenSpacePosition, matInvProj);
...
float3 CalcViewSpcaePosFromDepth(in float depth, in float2 uv, in float4x4 matInvProj)
{
	...
	float4 screenSpacePosition = float4(x, y, z, 1.f);
	float4 viewSpacePosition = mul(screenSpacePosition, matInvProj);
